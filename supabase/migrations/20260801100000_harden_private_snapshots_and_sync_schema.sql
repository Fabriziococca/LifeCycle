-- Keep the private safety snapshot protected even if schema grants change later.
alter table if exists private.user_data_snapshots enable row level security;
alter table if exists private.user_data_snapshots force row level security;
...
-- Keep the database allowlist aligned with every top-level key currently owned
-- by the cloud-first client. Server-owned scheduler keys remain excluded.
