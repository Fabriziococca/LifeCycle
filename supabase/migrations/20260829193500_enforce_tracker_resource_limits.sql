-- Enforce module and configurable-card quotas at the synchronized document
-- boundary. Client checks provide UX; this trigger is the non-bypassable guard.
...
begin
    ...
    if v_hygiene_stored is null or jsonb_typeof(v_hygiene_stored) = 'null' then
        return new;
    end if;
    ...
    if v_tracker_registry is null or jsonb_typeof(v_tracker_registry) = 'null' then
        return new;
    end if;
...
comment on function private.enforce_lifecycle_user_document_limit() is
    'Rejects oversized synchronized documents and tracker resources for bounded users; owner is unlimited.';
